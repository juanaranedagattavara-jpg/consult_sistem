...
export const serviceSchema = z.object({
  ...
  preAppointmentInstructions: z.string().max(1000, "Maximo 1000 caracteres").optional(), // SERV-03
});
    ...
    try {
      if (isEditing) {
        await updateService({
          ...
          price: data.price,
          color: data.color,
          preAppointmentInstructions: data.preAppointmentInstructions || undefined,
        });
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* Pre-Appointment Instructions (SERV-03) */}
            <FormField
              control={form.control}
              name="preAppointmentInstructions"
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Instrucciones pre-cita (opcional)</FormLabel>
                  <FormControl>
                    <Textarea
                      placeholder="Ej: No comer 2 horas antes, traer radiografias previas..."
                      className="min-h-[100px]"
                      {...field}
                    />
                  </FormControl>
                  <FormDescription>
                    Instrucciones que el paciente vera antes de la cita
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
...
- Pre-appointment instructions field (SERV-03)
